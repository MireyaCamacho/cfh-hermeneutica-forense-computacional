...
SECTIONS_CORPUS_B = [
    ("ENCABEZADO", [
        ...
        r"^(?:SALA\s+DE\s+RECONOCIMIENTO|SECCI[ÓO]N\s+DE)",
        r"^AUTO\s+N[Oº]\.",
        r"^REPÚBLICA\s+DE\s+COLOMBIA",
    ], False),

    ("ANTECEDENTES", [
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?ANTECEDENTES?(?:\s+(?:PROCESALES?|DEL?\s+CASO))?(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?HECHOS?\s+RELEVANTES?(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?CONTEXTO(?:\s+(?:GENERAL|F[ÁA]CTICO))?(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?OBJETO\s+(?:DEL?\s+AUTO|DE\s+LA\s+PROVIDENCIA)(?:\s*:|\s*\n)",
    ], False),

    # ZONA CRÍTICA: determinación de hechos y conductas — aquí está el lenguaje EBI
    ("HECHOS_Y_CONDUCTAS", [
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?DETERMINACI[ÓO]N\s+DE\s+(?:LOS\s+)?HECHOS\s+Y\s+CONDUCTAS(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?HECHOS\s+Y\s+CONDUCTAS(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?DESCRIPCI[ÓO]N\s+DE\s+(?:LOS\s+)?HECHOS(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?LOS\s+HECHOS(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?UNIVERSO\s+DE\s+(?:LAS\s+)?V[IÍ]CTIMAS(?:\s*:|\s*\n)",
    ], True),  # TARGET: narración de ejecuciones — alta densidad EBI/SA/NV

    # ZONA CRÍTICA: patrones macrocriminales — lenguaje de análisis sistémico
    ("PATRONES_MACROCRIMINALES", [
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?PATRONES?\s+(?:MACRO)?CRIMINALES?(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?MODALIDADES?\s+(?:DE\s+(?:LA\s+)?CONDUCTA|CRIMINAL)(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?MODUS\s+OPERANDI(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?SISTEMA\s+(?:DE\s+)?CRIMEN(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?PRÁCTICA\s+(?:SISTEM[ÁA]TICA|CRIMINAL)(?:\s*:|\s*\n)",
    ], True),  # TARGET: sistematización — lenguaje técnico-bélico institucional

    # ZONA CRÍTICA: calificación jurídica — eufemismos legales
    ("CALIFICACION_JURIDICA", [
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?CALIFICACI[ÓO]N\s+JUR[IÍ]DICA(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?TIPIFICACI[ÓO]N\s+(?:DE\s+(?:LAS?\s+)?CONDUCTAS?)?(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?CR[IÍ]MENES?\s+INTERNACIONALES?(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?CL[ÁA]SIFICACI[ÓO]N\s+(?:DE\s+(?:LOS\s+)?HECHOS)?(?:\s*:|\s*\n)",
    ], True),  # TARGET: transformación lingüística del hecho en categoría jurídica

    # ZONA TARGET: reconocimiento — indicador REP
    ("RECONOCIMIENTO", [
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?RECONOCIMIENTO\s+DE\s+(?:VERDAD|RESPONSABILIDAD)(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?DECLARACI[ÓO]N\s+DE\s+RECONOCIMIENTO(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?APORTE\s+A\s+LA\s+VERDAD(?:\s*:|\s*\n)",
        r"(?:^|\n)RECONOCE\s+(?:SU\s+)?RESPONSABILIDAD",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?COMPROMISOS?\s+(?:DE\s+)?RECONOCIMIENTO(?:\s*:|\s*\n)",
    ], True),  # TARGET: indicador REP — transición epistémica

    ("CONSIDERACIONES", [
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?CONSIDERACIONES?(?:\s+(?:DE\s+LA\s+SALA|JUR[IÍ]DICAS?))?(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?AN[ÁA]LISIS\s+JUR[IÍ]DICO(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?FUNDAMENTOS?\s+JUR[IÍ]DICOS?(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?MARCO\s+(?:JUR[IÍ]DICO|NORMATIVO)(?:\s*:|\s*\n)",
    ], True),

    ("RESUELVE", [
        r"(?:^|\n)RESUELVE(?:\s*:|\s*\n)",
        r"(?:^|\n)(?:[IVXLC]+\.?\s+)?EN\s+VIRTUD\s+DE\s+LO\s+EXPUESTO",

...
# Normalización facial por Z-SCORE sobre la distribución del corpus.
# Fundamento: a falta de un umbral de literatura para la intensidad absoluta de
# AUs en MediaPipe —cuya estimación la literatura reconoce como no confiable
# (Barrett et al., 2019)—, el ICM facial se normaliza de forma comparativa, que
# es como la literatura establece que se mide la intensidad facial. El z-score
# fue elegido empíricamente por discriminar SIN saturar (0/47 comparecientes en
# el techo, vs min-max y percentiles que saturan).
# Media y std CONGELADAS del distress crudo de los 47 comparecientes del corpus
# (calibración fija y reproducible: no se recalculan si entra un compareciente
# nuevo, garantizando estabilidad de la escala).
# distress = media(AU1,AU4,AU15,AU17); z = (d - mu)/sigma; norm = clip((z+3)/6, 0, 1).
FACIAL_MU = 0.1024        # media distress crudo del corpus (47 comparecientes)
FACIAL_SIGMA = 0.0369     # desviación estándar distress crudo del corpus
MIN_OVERLAP_S = 0.5
...
# Selección por dimensión (lo que mejor representa el habla del compareciente):
#   · shimmer → amean  : NIVEL de quiebre de voz (distress sostenido). Baird &
#                        Coutinho (2019): el quiebre marca el perdón sincero.
#   · F0      → stddevNorm : VARIABILIDAD tonal (arousal/expresividad). El tono
#                        medio (amean) NO sirve: depende de si el hablante tiene
#                        voz aguda/grave. Lo que marca emoción es cuánto FLUCTÚA.
#   · HNR     → amean (invertido) : NIVEL de calidad vocal. HNR bajo = voz
#                        aspirada/tensa sostenida = carga emocional.
#
# Umbrales NO dependen de la muestra (fijos y citables). Cada uno se ancla en:
#   - piso FISIOLÓGICO (cero perturbación / monotonía total), o
#   - techo de LITERATURA (umbral de irregularidad/aspiración documentado).
# Calibrados a la magnitud REAL de cada columna eGeMAPS v02 (verificado sobre el
# corpus C: shimmer_amean vive ~1.0–1.8; F0_stddevNorm ~0.05–0.20; HNR_amean ~2–10).
#
# shimmer_amean: piso 0.5 (umbral de irregularidad audible, Teixeira 2013), techo
#   1.8 (irregularidad severa). Mayor = más quiebre = más distress.
# F0_stddevNorm: adimensional. Piso 0.0 (monotonía total = fisiológico), techo 0.20
#   (alta variabilidad tonal ↔ arousal). Mayor = más expresivo.
# HNR_amean: voz limpia ~9–11 dB, aspirada ~2–4 dB (literatura calidad vocal). Piso
#   efectivo 11 (limpia), techo 2 (aspirada). INVERTIDO: menos HNR = más distress.
VOCAL_ESCALA = {
    "shimmerLocaldB_sma3nz_amean":            (0.5,  1.8,  False),  # (lo, hi, invertir)
    "F0semitoneFrom27.5Hz_sma3nz_stddevNorm": (0.0,  0.20, False),
    "HNRdBACF_sma3nz_amean":                  (2.0,  11.0, True),
}
...
def facial_por_persona(base, prefijo, intervalos, diar):
    ...
    # Normalización por Z-SCORE sobre la distribución del corpus (media/std
    # congeladas en FACIAL_MU/FACIAL_SIGMA). La unidad de análisis es el
    # compareciente individual; el z-score lo posiciona respecto a la media del
    # corpus, de forma comparativa entre comparecientes (no mide distress
    # absoluto, que la literatura considera no confiable —Barrett et al. 2019).
    # Mapeo z∈[-3,3] → [0,1] vía (z+3)/6, con clip. Misma regla para todos.
    if not distress_crudo:
        return {}
    res = {}
    for ident, (dist, n) in distress_crudo.items():
        z = (dist - FACIAL_MU) / FACIAL_SIGMA if FACIAL_SIGMA > 1e-9 else 0.0
        norm = float(np.clip((z + 3) / 6, 0, 1))
        res[ident] = (norm, n)
    return res
...
def procesar(subcaso, base, rep_ext):
    ...
    # --- VOCAL corregido ---
    vocal = {}
    # Buscar el eGeMAPS: primero egemap_<pref>.csv, luego _compareciente
    # (Costa Caribe lo tiene como egemap_costa_caribe_compareciente.csv).
    cand_eg = [base / "outputs" / "capa3" / f"egemap_{pref}.csv",
               base / "outputs" / "capa3" / f"egemap_{pref}_compareciente.csv"]
    p_eg = next((p for p in cand_eg if p.exists()), None)
    if p_eg is not None:
        eg = pd.read_csv(p_eg)
        # Aceptar 'start_s'/'end_s' o 'start'/'end' como columnas temporales
        ren = {}
        if "start_s" not in eg.columns and "start" in eg.columns:
            ren["start"] = "start_s"
        if "end_s" not in eg.columns and "end" in eg.columns:
            ren["end"] = "end_s"
        if ren:
            eg = eg.rename(columns=ren)
...
def main():
    ...
    print("  Normalización facial: Z-SCORE sobre corpus (mu/sigma congeladas, comparativa)")
